...
Axiom paths_next : forall (m:(map vertex dist)) (d:Z), ((0%Z <= d)%Z /\
  (d <  (cardinal vertices))%Z) -> ((paths m d edges) -> (paths m (d + 1%Z)%Z
...
Theorem WP_parameter_bellman_ford : (paths (set (const Infinite:(map vertex
  ...
  (paths m (((cardinal vertices) - 1%Z)%Z + 1%Z)%Z (empty :(set1 (vertex*
  vertex)%type))) -> forall (e:(set1 (vertex* vertex)%type)),
  forall (result:bool), ((result = true) <-> (is_empty e)) ->
  ((result = true) -> forall (v:vertex), (mem v vertices) -> forall (n:Z),
  (exists d:Z, (shortest_path s v n d)) -> ((get m v) = (Finite n)))).
intuition.
