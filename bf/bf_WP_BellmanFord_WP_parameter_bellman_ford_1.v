...
Theorem WP_parameter_bellman_ford : (1%Z <= ((cardinal vertices) - 1%Z)%Z)%Z ->
  ...
  ((result = true) <-> (is_empty es)) -> ((result = true) -> ((inv1 m
  (cardinal vertices) (empty :(set1 (vertex* vertex)%type))) -> ((inv2 m
  edges) -> forall (v:vertex), (mem v vertices) -> match (get m
  v) with
...
unfold inv1 in H6.
unfold inv2 in H7.
assert (H10 : mem v vertices) ; auto.
assert (H11 : mem v vertices) ; auto.
apply H6 in H10.
apply H7 in H11.
